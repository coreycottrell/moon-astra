...
Install backup.py as `/usr/local/sbin/moon-astra-v2-backup` (root-owned 0755) and the matching backup service/timer under `/etc/systemd/system`. Enable `moon-astra-v2-backup.timer`; start its service once to verify. Each hourly run snapshots both v2 databases through SQLite's online backup API into a new private `/var/backups/moon-astra-v2/<UTC>/` directory, checks integrity, schema and identity counts, and records source revisions and SHA-256 values. `latest.json` locates the newest pair. There is no automatic deletion; monitor disk space and copy snapshots off the VPS. Original Moon backup jobs are independent.
