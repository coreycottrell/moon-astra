...
export const BUILDINGS={
  ...
  depot:{name:'Freight depot',cost:20,parts:2,power:-1,radius:5,mind:0,work:180,description:'Warehouse with a reserved six-bay apron, two initial lift connections, two loading positions and 240 units of storage. Research unlocks physical expansion to four and six connections.'},
...
export const TECH={
  ...
  'tunnel-control':{name:'Tunnel traffic control',cost:1200,requires:['tunneling'],building:'depot',description:'Schedule three-robot convoys and unlock physical depot expansion from two to four lift bays.'},
  'tunnel-passing':{name:'Underground passing bays',cost:1800,requires:['tunnel-control'],description:'Fit passing chambers for four robots with opposing traffic, and expand depots to six lift bays.'},
  'tunnel-twins':{name:'Twin freight tubes',cost:2600,requires:['tunnel-passing','thermal-design'],description:'Excavate independent lanes for four robots in each direction. Each terminus still has one elevator.'},
...
export const ACTIONS=['build.place','build.cancel','blueprint.deploy','machine.configure','machine.pause','replicator.configure','replicator.order','replicator.stop','robot.fabricate','robot.recondition','crew.configure','crew.lend','research.select','design.certify','design.apply','freight.transfer','shipment.send','project.contribute','agent.request','board.post','board.reply','board.close','tunnel.dig','tunnel.upgrade','tunnel.fitout','tunnel.cancel','depot.expand','depot.assign','claim.pause','claim.grant','claim.revoke'];
