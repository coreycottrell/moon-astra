...
import {loadMachineAssets,MACHINE_TYPES} from './machine-assets.js';
...
function show(type,collection=false){
 ...
 const types=all?MACHINE_TYPES:[type];types.forEach((t,i)=>{const g=createMachine(t),x=all?(i%3-1)*14:0,z=all?(Math.floor(i/3)-.5)*14:0;g.position.set(x,.08,z);g.userData.galleryType=t;scene.add(g);machines.push(g);pedestal(x,z,all?5.8:6.3);if(all){const label=document.createElement('div');label.className='model-label';label.textContent=descriptions[t].name.toUpperCase();$('labels').append(label);}});
 const d=descriptions[type];$('model-name').textContent=d.name;$('category').textContent=d.category;$('model-description').textContent=d.body;$('specs').innerHTML=d.specs.map(([a,b])=>`<div class="spec"><span>${a}</span><b>${b}</b></div>`).join('');
 camera.position.set(...(all?[26,33,42]:innerWidth<620?[14,12,21]:['miner','refinery','compute'].includes(type)?[10,8,13]:[13,10,17]));controls.target.set(all?0:innerWidth<1000?0:2.3,all?0:2.0,0);controls.maxDistance=all?100:45;controls.update();
 document.querySelectorAll('.model-tab').forEach(b=>b.classList.toggle('active',b.dataset.type===type&&!all));$('lineup').textContent=all?'Inspect selected machine ↗':'View complete collection ↗';
 $('status').textContent=state==='working'?'LIVE MECHANICAL PREVIEW':state==='paused'?'PAUSED / MOTION HELD':'SUPERVISION NEEDED / MOTION HELD';
}
$('model-nav').innerHTML=MACHINE_TYPES.map((t,i)=>`<button class="model-tab" data-type="${t}"><span>0${i+1} / ${t==='compute'?'MIND NODE':t==='miner'?'HARVESTER':t.toUpperCase()}</span><b>${descriptions[t].name}</b></button>`).join('');
...
try{const loaded=await loadMachineAssets((n,total)=>$('status').textContent=`PREPARING ${n} / ${total}`);if(loaded.failed.length)throw Error('Some detailed models could not be loaded');ready=true;show(Object.hasOwn(descriptions,new URLSearchParams(location.search).get('model'))?new URLSearchParams(location.search).get('model'):'seed',new URLSearchParams(location.search).has('all'));
