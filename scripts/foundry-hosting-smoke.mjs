...
const base='/moon-foundry/',root=resolve('dist-aiciv');
...
try{
  ...
  await page.goto(url+'machines.html?model=titan');await page.getByRole('heading',{name:'Titan',exact:true}).waitFor({timeout:90000});await page.screenshot({path:'artifacts/foundry/browser-hosted-prefix.png'});
  ...
  const report={ok:true,mount:base,checks:['compiled game and proxied identity','16-model gallery and Titan','whitepaper cross-reference page','agent manual','extensionless document paths'],errors,badResponses:bad};mkdirSync('artifacts/foundry',{recursive:true});writeFileSync('artifacts/foundry/hosting-smoke.json',JSON.stringify(report,null,2)+'\n');console.log(JSON.stringify(report,null,2));
