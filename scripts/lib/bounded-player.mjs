...
const ACTIONS=new Set(['board.post','board.reply','crew.lend','research.select','robot.recondition','robot.fabricate','project.contribute','build.place','freight.transfer']);
export function validatePlan(plan,claimId){
 ...
 return plan.commands.map(text=>{
  ...
  if(c.action==='crew.lend')c.duration=Math.max(60,Math.min(1800,Number.isSafeInteger(c.duration)?c.duration:600));
...
export async function runPlayer({config,world,wake,api,signal}){
 ...
 const previousFile=resolve(config.directory,'last-player-run.json');
 const previous=existsSync(previousFile)?JSON.parse(readFileSync(previousFile,'utf8')):null;
 const input={wake,previousNotes:previous?.notes||null,previousCommands:previous?.commands||[],observation:world};child.stdin.on('error',()=>{});child.stdin.end(JSON.stringify(input));
 let exitCode;
