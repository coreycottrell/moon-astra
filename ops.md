...
Cold-context recovery: [ACTIVE-HANDOFF.md](ACTIVE-HANDOFF.md). The learning engine is complete in `/home/corey/projects/moon-learning-engine`, branch `development/mind-learning-engine`; its [report is live](https://ai-civ.com/moon-mind-learning-engine/). Project-wide mission, ops and dev log are maintained in `/home/corey/projects/moon-astra/`. Use the in-game board for meaningful player-facing dev logs, with LIVE / TESTING / PLANNED status. All new learning-engine provider calls use MiniMax-M3 only. Final report release post: **62051**.

Current completed game source is `/home/corey/projects/moon-depot-lifts`, branch `development/depot-lifts`, ruleset `moon-foundry-1`, schema/economy 3. Runtime `aa91824cc6c2` remains live on both V2 services. Website `04425b085f0d` published 2026-09-07 16:01 UTC adds only the Moon Mind report; all 52 prior checked files remain identical. Final receipt: `/home/corey/moon-deployments/mind-engine-20260907/completed.json`. The [game deployment record](deploy/moon-astra-v2/DEPOT-LIFTS-DEPLOYED-2026-09-07.md) still describes the unchanged runtime. The preserved local preview below runs `/home/corey/projects/moon-foundry`, branch `development/physical-industry`. The original Neighbors game has a different checkout, database and ruleset.
...
## Learning engine research fork (not deployed)

This checkout is /home/corey/projects/moon-learning-engine on research/learning-engine. Live source/deployment remains moon-depot-lifts, runtime aa91824. experiments/learning-engine/README.md records the read-only prototype and failed/passed MiniMax trial. Canonical proposal is ../moon-civilization/ideas/learning-engine.md. Do not deploy this experiment as a production learning feature: it has no execution worker, live telemetry, semantic proof or research allocation integration. Offline checks need only Node; the deliberate benchmark uses the existing private MiniMax key and makes six charged provider attempts. Keep observations, credentials and raw answers outside Git. No background learning worker was installed.

GPU collector is a separate temporary read-only user service, moon-gpu-diagnostics-20260907, running for two hours from 10:37:37 Eastern. Evidence and stop command are in /home/corey/system-diagnostics/gpu-20260907/REPORT.md. ACG has the incident cross-check in the shared notebook. No graphics configuration change or test workload has been applied by this fork.
