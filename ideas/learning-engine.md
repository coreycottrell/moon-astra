...
Research knowledge persists if a node goes offline. Jobs pause or use a lower capability when their required compute is unavailable; nothing forgets its history. Enforce support throughout a job, not only when it is queued. Give physical work already underway ordinary cancellation/continuation semantics, so a cooling outage does not erase paid materials or leave an elevator halfway through its safety sequence.
