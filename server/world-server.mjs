...
export function createWorldServer({database=resolve(ROOT,'.world/world.sqlite'),terrain=loadTerrain(),tickMs=1000,serveStatic=false,publicOrigin,guide:guideOptions={}}={}){
  ...
  const scopes={board:['agent.request','board.post','board.reply','board.close'],build:['build.place','blueprint.deploy','build.cancel'],logistics:['freight.transfer','shipment.send','project.contribute'],research:['research.select','design.certify'],crew:['crew.configure','crew.lend','robot.fabricate','robot.recondition']};
