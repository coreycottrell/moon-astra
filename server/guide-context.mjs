...
import {tunnelStatus,occupiedPorts,depotFree} from '../src/foundry/lift-network.js';
...
export const GUIDE_RULES={
  ...
  buildOrders:'In Industry, replicators accept finite ordered steps with quantities. Coordinated construction research unlocks repeating cycles and support-first group templates. They wait for each site to commission; waiting releases the replicator fabrication mind. Cancelling that site stops the order. Stop future builds preserves a paid kit and existing site. Daughter replicators from orders start off. Estimates include 0.25 mind per supervised crew member and full-load enabled industry; they assume grid connection and do not guarantee cooling, supply, placement or hauling. Templates are fixed sequences, not adaptive planners.',
  ...
  tunnels:'Industry > Plan a tunnel requires explicit Start and End; the assigned bore only excavates. Local routes are 20–500 m; neighbor seeds up to 6 km. Liners cost 0.5 metal + 0.1 parts and 10 powered seconds per meter. Both elevators cost 12 metal + 4 parts reserved at order time, then physically delivered and commissioned after excavation. A basic line carries one robot total until it clears the exit. Four seconds to lower, 1.5x condition-adjusted underground travel, four seconds to call the destination platform down, four to rise; queues and crew limits add time. Cargo stays aboard and robots keep their mind cost. Scheduled convoys research permits paid capacity 3 same-direction upgrades; Passing bays permits capacity 4 with reserved half-sections; Twin tunnels permits capacity 8, 4 per direction, sharing each terminus. Existing utility links need the explicit Fit 2 elevators action; endpoints remain unchanged. Stop this excavation only cancels unfinished routes with ordinary supply return/salvage. Neighbor connections do not grant inventory access or share power.',
  depots:'New depots reserve a 19 m radius apron for six fixed terminus bays. Two bays start usable; paid robot-built expansions to 4 and 6 require Scheduled convoys and Passing bays research respectively, each costing 16 metal + 4 parts. Old depots need a clear-ground apron retrofit first and retain their previous storage until the player requests it. Crowded old depots are preserved; place another depot on clear ground. New hubs hold 240 units total including reserved incoming freight and have two physical loading positions with 3 seconds handling. Connected producers send surplus to their assigned depot and prefer it for input supplies; auto-assignment occurs when the first producer/depot lift pair commissions. Targets are rock 120, metal 80, parts 24 and spares 16. Every packet still needs a robot. Suggested links fill the planner for review and do not issue orders.',
...
export function guideContext(w,actor,{machineId,robotId}={}){
  ...
  const machine=m=>({
    ...take(m,['id','type','claimId','ownerId','enabled','mode','design','generation','productionStatus','pendingBuild','buildOrder','serviceBy','depotId','depotHub']),
    ...
    upgrade:m.upgrade||null,depot:m.type==='depot'?{bays:m.depotHub?.bays||0,reserved:occupiedPorts(w,m).length,freeStorageUnits:m.depotHub?depotFree(w,m)/UNIT:null}:undefined,
  });
  const crew=r=>({...take(r,['id','name','role','claimId','workClaimId','status','task','serviceBy','reconditioning','loanUntil','tunnelRide','undergroundDepth']),conditionPercent:r.condition/100,distanceTravelledMetres:Math.round(r.distanceTravelled),cargo:(r.cargo||[]).map(p=>({...p,amount:p.amount/UNIT})),routeWaypointsRemaining:r.path?.length||0});
  ...
  return {
    ...
    construction:w.jobs.filter(j=>j.claimId===claim.id).slice(0,100).map(j=>({...take(j,['id','type','phase','stage','crew','duration','remaining','prefab','label','infrastructure']),inventory:units(j.inventory),cost:units(j.cost),work:j.work/UNIT,stageWork:j.stageWork.map(n=>n/UNIT)})),
    ...
    metalFlow:{
      ...
      metalInTunnelInfrastructure:w.corridors.filter(t=>t.claimId===claim.id).reduce((n,t)=>n+(t.infrastructureCost?.metal||0)+t.excavated*500,0)/UNIT,
      metalInInstalledBuildings:own.reduce((n,m)=>n+(m.embodied?.metal||0)+(m.infrastructureCost?.metal||0),0)/UNIT,
    ...
    corridors:w.corridors.filter(t=>t.claimId===claim.id||t.targetClaimId===claim.id).map(t=>({...take(t,['id','fromId','toId','targetClaimId','length','excavated','complete','transport','liftVersion','tier','terminals','upgradeJobId']),status:tunnelStatus(w,t),installedInfrastructureCost:units(t.infrastructureCost),remainingMetal:(t.length-t.excavated)*.5,remainingParts:(t.length-t.excavated)*.1,robotsOnRoute:w.robots.filter(r=>r.tunnelRide?.corridorId===t.id).length})),
