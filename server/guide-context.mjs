...
export const GUIDE_RULES={
  ...
  tunnels:'Bores connect own facilities within 500 m or a neighboring seed within 6 km. New corridors open two robot freight lanes only after excavation completes; 0.5 metal + 0.1 parts and 10 powered seconds per meter. Robots retain cargo, mind costs and physical travel; smooth lanes give 1.5x travel speed. Existing utility-only corridors keep their power/data role. Neighbor links do not merge inventories, grant access or share power. Industry shows cost, endpoint and progress. Local completed corridors also extend local utility connectivity.',
...
export function guideContext(w,actor,{machineId,robotId}={}){
  ...
  const machine=m=>({
    ...take(m,['id','type','claimId','ownerId','enabled','mode','design','generation','productionStatus','pendingBuild','buildOrder','serviceBy']),
  ...
  return {
    ...
    corridors:w.corridors.filter(t=>t.claimId===claim.id||t.targetClaimId===claim.id).map(t=>({...take(t,['id','fromId','toId','targetClaimId','length','excavated','complete','transport']),remainingMetal:(t.length-t.excavated)*.5,remainingParts:(t.length-t.excavated)*.1,robotsOnRoute:w.robots.filter(r=>r.tunnelRide?.corridorId===t.id).length})),
