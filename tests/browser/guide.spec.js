...
test('in-game guide preserves drafts, polls private answers, escapes model text and stays separate from commands',async({page,request})=>{
 ...
 await page.locator('.guide-messages').evaluate(el=>el.scrollTop=180);await page.getByRole('heading',{name:'Ask about your Moon'}).click();await page.waitForTimeout(1800);
 expect(await page.locator('.guide-messages').evaluate(el=>el.scrollTop)).toBeCloseTo(180,0);
