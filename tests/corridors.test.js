...
test('owners preview and excavate toward neighboring seeds without resource or permission grants',()=>{
 ...
 assert.deepEqual(w.machines.map(m=>m.inventory),before.machines.map(m=>m.inventory));assert.deepEqual(w.claims.map(c=>c.builders),before.claims.map(c=>c.builders));
...
test('the working bore and route endpoints are independent, and only the bore pays excavation costs',()=>{
 ...
 for(let n=0;n<800&&!route.complete;n++)stepWorld(w);assert.ok(route.complete);assert.equal(bore.inventory.metal,100000-route.length*500);assert.deepEqual(seed.inventory,before);
});
...
test('completed routes transport real cargo, survive restart, and deliver only after emerging',()=>{
 ...
 cmd('tunnel.dig',{machineId:from.id,toId:to.id});const tunnel=w.corridors[0];tunnel.excavated=tunnel.length;tunnel.complete=true;
