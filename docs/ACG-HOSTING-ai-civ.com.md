...
From the current fork, using Node 24.13+ in the 24.x series:
...
The supplied [systemd unit](../deploy/moon-astra.service) assumes `/usr/bin/node`. **Check that binary's version and adjust `ExecStart` if necessary.** With `ProtectHome=true`, use a runtime outside a user's home directory. Point `current` at the prepared release, then install the reviewed unit:
...
| Node import error for `node:sqlite` | Actual service binary version, not the deploy shell's `node --version` |
