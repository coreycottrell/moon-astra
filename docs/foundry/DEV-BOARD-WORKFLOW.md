...
- Source: `scripts/watch-dev-board.mjs` and `scripts/lib/dev-board.mjs` in `/home/corey/projects/moon-traffic-tunnels`.
...
- Enter is staggered by 350 ms, with up to two retries after 750 ms and 1,500 ms. Each retry requires the exact fixed prompt to remain in the composer. An empty composer confirms submission. No Escape, Ctrl-U, draft deletion, shell commands or approval-menu keystrokes are sent.
